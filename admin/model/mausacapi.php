...
class mausacapi{
	...
	public function readbypage() {
	    ...
	    $countQuery = "SELECT COUNT(*) as total FROM mausac WHERE hienthi=:hienthi";

	    if (!empty($this->search)) {
	        $countQuery .= " AND tenmau LIKE :search";
	    }
	    ...
	    $query = "SELECT * FROM mausac WHERE hienthi=:hienthi";
	...
	public function show(){
		$query = "SELECT * FROM mausac where id=? limit 1";
		$stmt = $this->conn->prepare($query);
		...
		$this->id = $row['id'];	
		$this->tenmau = $row['tenmau'];
		$this->hienthi = $row['hienthi'];
	}

	public function create(){
		$query = "INSERT INTO mausac SET tenmau=:tenmau, hienthi=:hienthi";
		$stmt = $this->conn->prepare($query);

		$this->tenmau = $this->tenmau;
		$this->hienthi = $this->hienthi;

		$this->hienthi = isset($this->hienthi) ? $this->hienthi : '0';

		$stmt->bindParam(':tenmau',$this->tenmau);
	...
	public function update(){
		$query = "UPDATE mausac SET tenmau=:tenmau, hienthi=:hienthi WHERE id=:id";
		$stmt = $this->conn->prepare($query);
	...
	public function updatestatus(){
		$query = "UPDATE mausac SET hienthi=:hienthi WHERE id=:id";
		$stmt = $this->conn->prepare($query);
