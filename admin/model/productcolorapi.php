...
class productcolorapi{
	...
	public function read(){
		$query = "SELECT * FROM sanphammausac";
	...
	public function readbypage() {
	    ...
	    $countQuery = "SELECT COUNT(*) as total FROM sanphammausac WHERE hienthi=:hienthi AND masp = :filtersanpham";
	    ...
	    $query = "SELECT spt.*,ms.tenmau,kt.tenkichco FROM sanphammausac spt inner join mausac ms on spt.mausac=ms.id inner join kichco kt on spt.kichco=kt.id WHERE spt.hienthi=:hienthi AND spt.masp = :filtersanpham";
	...
	public function show(){
		$query = "SELECT * FROM sanphammausac where id=? limit 1";
		$stmt = $this->conn->prepare($query);
	...
	public function isURLExists($url,$color) {
	    $query = "SELECT COUNT(*) as count FROM sanphammausac WHERE url = :url AND mausac = :color ";
	    $stmt = $this->conn->prepare($query);
	...
	public function create(){
		if ($this->isURLExists($this->url,$this->mausac)) {
	        printf("Error: Đã tồn tại.\n");
	        return false;
	    }
		$query = "INSERT INTO sanphammausac SET masp=:masp, tensp=:tensp, hinhanh=:hinhanh, gia=:gia, giakm=:giakm, mausac=:mausac, kichco=:kichco, uutien=:uutien, soluong=:soluong, thoigian=:thoigian, hienthi=:hienthi, url=:url";
		$stmt = $this->conn->prepare($query);
	...
	public function update(){  
	    $query = "UPDATE sanphammausac SET gia=:gia, giakm=:giakm, mausac=:mausac, kichco=:kichco, uutien=:uutien, soluong=:soluong, thoigian=:thoigian, hienthi=:hienthi";
	...
	public function delete(){
		$query = "DELETE FROM sanphammausac 
		WHERE id=:id";
		$stmt = $this->conn->prepare($query);
	...
	public function updatestatus(){
		$query = "UPDATE sanphammausac SET hienthi=:hienthi WHERE id=:id";
		$stmt = $this->conn->prepare($query);
	...
	public function updateproductsize(){  
	    if (isset($this->hinhanh) && $this->hinhanh != "undefined") {
	        $query = "UPDATE sanphamkichco SET hinhanh=:hinhanh WHERE masp=:masp AND mausac=:mausac";
	        $stmt = $this->conn->prepare($query);
	        $this->hinhanh = $this->hinhanh;
	        $stmt->bindParam(':hinhanh', $this->hinhanh);
	        $this->masp = $this->masp;
